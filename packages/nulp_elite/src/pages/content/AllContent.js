      ...
      {Object?.entries(
        ...
        <React.Fragment key={category}>
          <p style={{display:'flex',justifyContent:'space-between'}}>
           <Box style={{display:'inline-block',fontSize:'14px',color:'#1E1E1E'}}><SummarizeOutlinedIcon style={{verticalAlign:'top'}}/> <Box style={{borderBottom:'solid 2px #000'}}>{category}{" "} </Box> </Box>
            {items?.length > 4 && (
              <Link
                to={`/view-all/${category}`}
                style={{ color: "#424242", fontSize: "12px",textAlign:'right',fontWeight:'600' }}
              >
                View All
              </Link>
            )}
