...
const Registration = () => {
  ...
  useEffect(() => {
    const getOtp = async () => {
      ...
      try {
        const response = await contentService.getOtpResponse(
          url,
          data,
          headers
        );
...
import { Select, MenuItem, MenuList } from "@chakra-ui/react";
const options = ["Option 1", "Option 2", "Option 3", "Option 4"];
const DropdownWithAllOption = () => {
...
// export default DropdownWithAllOption;
...
function UserPrefPopup() {
  ...
  useEffect(() => {
    if (isOpen) {
      // Call your API here to fetch categories
  ...
  const handleCategoryChange = async (categoryValue) => {
    // Call your API here to fetch sub-categories based on the selected category
    try {
      const subCategoriesData = await fetchSubCategoriesFromAPI(categoryValue);
  ...
  const fetchSubCategoriesFromAPI = async (categoryValue) => {
    // Example API call using fetch
    const response = await fetch(`your-api-endpoint/${categoryValue}`);
    if (!response.ok) {
